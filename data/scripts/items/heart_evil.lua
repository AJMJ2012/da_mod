...
--[[
	...
	EntityLoad("mods/da_mod/files/entities/particles/image_emitters/heart_effect_evil.xml", x, y-12)
	EntityLoad("mods/da_mod/files/entities/particles/heart_out_evil.xml", x, y-8)
	local description = GameTextGet( "$logdesc_heart_evil", tostring(math.floor((max_hp - max_hp_old) * 25)), tostring(math.floor(hp * 25)) )
	if max_hp == max_hp_old then
		description =  GameTextGet( "$logdesc_heart_blocked", tostring(math.floor(hp * 25) - math.floor(hp_old * 25)))
	elseif GlobalsGetValue( "GLASS_CANNON_HP_DIVISOR", "0" ) == 0 then
